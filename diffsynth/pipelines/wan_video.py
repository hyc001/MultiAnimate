...
def generate_label_map(mask_a, mask_b, mask_c):
    """
    根据传入的三个 mask 生成 label map：
    背景为 0，mask_a 对应标签 1，mask_b 对应标签 2，mask_c 对应标签 3
    """
    # 判断每个 mask 的有效区域
    m_a = (mask_a > 127.5).any(dim=1, keepdim=True).float()  # (Batch, 1, 81, H, W)
    m_b = (mask_b > 127.5).any(dim=1, keepdim=True).float()
    m_c = (mask_c > 127.5).any(dim=1, keepdim=True).float()

    # 初始化 label_map，初始值全为 0（背景）
    label_map = torch.zeros_like(m_a, dtype=torch.long)  # (Batch, 1, 81, H, W)

    # 依次赋值（后出现的会覆盖前者的同位置像素）
    label_map[m_a == 1] = 1
    label_map[m_b == 1] = 2
    label_map[m_c == 1] = 3

    # One-hot 编码（类别总数=4：背景 + 3个人）
    label_map_one_hot = torch.nn.functional.one_hot(label_map.squeeze(1), num_classes=4)  # (Batch, 81, H, W, 4)
    print(f"after one hot: {label_map_one_hot.shape}")

    # 调整维度顺序，便于后续输入模型
    label_map_one_hot = label_map_one_hot.permute(0, 4, 1, 2, 3)  # (Batch, 4, 81, H, W)

    return label_map_one_hot

def generate_label_map1(mask_a, mask_b, num_classes=4, thresh=127.5, seed=None):
    """
    用双人 mask 生成 num_classes 通道的 one-hot 标签，用于数据增强：
      - 随机从 [0, num_classes-1] 中选择3个通道编号：
        分别对应 背景 / 人物A / 人物B
      - 其他通道为空（全 0）
      - 若 A 和 B 重叠，则人物B优先覆盖人物A

    参数:
      mask_a, mask_b: torch.Tensor, 形状 (B, C, T, H, W)
      num_classes: 输出标签通道数 (默认 4)
      thresh: 像素阈值
      seed: 随机种子 (为了复现)

    返回:
      label_map_one_hot: torch.Tensor, (B, num_classes, T, H, W)
      mapping: dict，记录随机分配关系 {'background': int, 'person_a': int, 'person_b': int}
    """

    if seed is not None:
        random.seed(seed)

    # 1️⃣ 提取二值 mask (合并颜色通道)
    m_a = (mask_a > thresh).any(dim=1, keepdim=True)   # (B,1,T,H,W)
    m_b = (mask_b > thresh).any(dim=1, keepdim=True)  # (B,1,T,H,W)
    B, _, T, H, W = m_a.shape

    # 2️⃣ 随机挑选3个通道编号
    chosen = random.sample(range(num_classes), 3)
    bg_id, a_id, b_id = chosen

    # 3️⃣ 初始化 label 索引图
    # torch.full 创建一个固定值填充的张量
    # 这里生成全是背景标签的张量 (B,1,T,H,W)
    label_idx = torch.full(
        (B, 1, T, H, W),
        fill_value=bg_id,
        dtype=torch.long,
        device=m_a.device
    ...
    # 4️⃣ 标注人物 A 和 B
    label_idx[m_a] = a_id
    label_idx[m_b] = b_id  # 冲突位置B会覆盖A

    # 5️⃣ 转换为 one-hot
    label_map_one_hot = F.one_hot(label_idx.squeeze(1), num_classes=num_classes)  # (B,T,H,W,C)
    label_map_one_hot = label_map_one_hot.permute(0, 4, 1, 2, 3).float()          # (B,C,T,H,W)
    print(f"after one hot:{label_map_one_hot.shape}")
    # 6️⃣ 返回 one-hot 标签 + 通道映射
    # mapping = {"background": bg_id, "person_a": a_id, "person_b": b_id}

    return label_map_one_hot

# def generate_label_map(mask_a, mask_b):

#     m_a = (mask_a > 127.5).any(dim=1, keepdim=True).float()  # (Batch, 1, 81, H, W)
#     m_b = (mask_b > 127.5).any(dim=1, keepdim=True).float()  # (Batch, 1, 81, H, W)
...
class WanUniAnimateVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        ####
        # self.label_embedding = nn.Conv3d(3, 3, kernel_size=1)
        # define the additional modules
        concat_dim = 4
        # self.label_embedding = nn.Conv3d(3, 3, kernel_size=1)  # (Batch, 3, 81, H, W)
        ...
        for key in model_manager.state_dict_new_module:
            if "label_embedding" in key:
                state_dict_new[key.split("label_embedding.")[1]] = model_manager.state_dict_new_module[key]
            # else:
            #     print("no key")
        self.label_embedding.load_state_dict(state_dict_new,strict=True)
        self.label_embedding.to(torch.bfloat16)
    ...
    def __call__(
        ...
        self.dwpose_embedding.to(self.device)
        self.randomref_embedding_pose.to(self.device)
        self.label_embedding.to(self.device)

        f_mask = f_mask.unsqueeze(0)
        m_mask = m_mask.unsqueeze(0)

        if c_mask is not None:
            c_mask = c_mask.unsqueeze(0)
            label_map = generate_label_map(f_mask, m_mask, c_mask).to(torch.bfloat16)
        
        else:
            label_map = generate_label_map1(f_mask, m_mask).to(torch.bfloat16)


        dwpose_data = dwpose_data.unsqueeze(0)
        # labeled_dwpose = dwpose_data / 255. + label_embedding_output
        # dwpose_data = self.dwpose_embedding((torch.cat([labeled_dwpose[:,:,:1].repeat(1,1,3,1,1), labeled_dwpose], dim=2)/255.).to(self.device)).to(torch.bfloat16)
        
        label_embedding_output = self.label_embedding((torch.cat([label_map[:,:,:1].repeat(1,1,3,1,1), label_map], dim=2)).to(self.device)).to(torch.bfloat16)
        dwpose_data_temp = self.dwpose_embedding((torch.cat([dwpose_data[:,:,:1].repeat(1,1,3,1,1), dwpose_data], dim=2)/255.).to(self.device)).to(torch.bfloat16)
        dwpose_data = label_embedding_output + dwpose_data_temp
