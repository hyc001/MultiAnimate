...
def generate_label_map(mask_list, num_classes=4, thresh=127.5):
    """
    Generates an (N + 1)-channel one-hot label map from multiple masks.

    - Each binarized `mask_i` is assigned a class ID `i` (where 1 <= i <= N).
    - The background is assigned the class ID `0`.

    Args:
        mask_list (List[torch.Tensor]): A list of length N. Each tensor has a shape 
            of (C, T, H, W) without a batch dimension (assuming B=1).
        num_classes (int): The maximum number of characters supported by the generation 
            pipeline (i.e., the maximum support channels).
        thresh (float, optional): The pixel threshold value. Masks are binarized 
            such that pixels > thresh are considered foreground. Defaults to 127.5.

    Returns:
        Tuple[torch.Tensor, dict]:
            - label_map_one_hot: The one-hot encoded label map of shape (1, N+1, T, H, W).
            - mapping: A dictionary mapping class names to their corresponding IDs:
              {
                  'person_1': 1,
                  'person_2': 2,
                  ...,
    ...
    """
    assert len(mask_list) > 0, "mask_list cant't be none"
    N = len(mask_list)  

    # per-person mask shape
    C, T, H, W = mask_list[0].shape
    ...
    for i, mask in enumerate(mask_list):
        # mask: (C, T, H, W)
        m = (mask > thresh).any(dim=0, keepdim=True)  # (1, T, H, W)

        # 2) (B,1,T,H,W)，B=1
        m = m.unsqueeze(0)  # (1, 1, T, H, W)

        # 3) set label_idx 
        label_idx[m] = i+1

    # one-hot: (B, 1, T,H,W) → (B, T,H,W, num_classes) → (B, num_classes, T,H,W)
...
class WanVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        # ### recommond text feature cache when trained on A100
...
class WanUniAnimateVideoPipeline(BasePipeline):
    ...
    def __init__(self, max_p=3, device="cuda", torch_dtype=torch.float16, tokenizer_path=None):
        super().__init__(device=device, torch_dtype=torch_dtype)
        self.p_channnel = max_p + 1
        self.scheduler = FlowMatchScheduler(shift=5, sigma_min=0.0, extra_one_step=True)
        self.prompter = WanPrompter(tokenizer_path=tokenizer_path)
        self.text_encoder: WanTextEncoder = None
        self.image_encoder: WanImageEncoder = None
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.label_embedding = nn.Sequential(
                    nn.Conv3d(self.p_channnel, concat_dim * 4, (3,3,3), stride=(1,1,1), padding=(1,1,1)),
                    nn.SiLU(),
    ...
    @staticmethod
    def from_model_manager(model_manager: ModelManager, max_p=3, torch_dtype=None, device=None, use_usp=False):
        if device is None: device = model_manager.device
        if torch_dtype is None: torch_dtype = model_manager.torch_dtype
        pipe = WanUniAnimateVideoPipeline(max_p=max_p, device=device, torch_dtype=torch_dtype)
        pipe.fetch_models(model_manager)
    ...
    def __call__(
        ...
        label_map = generate_label_map(masks, num_classes=self.p_channnel).to(torch.bfloat16)
